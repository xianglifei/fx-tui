...
| `Ctrl+T` | 思考过程 展开⇄收起：思考中实时尾随显示，空闲时打印上一步思考全文 |
...
- **复制回复**：`/copy` 把最后一条助手回复的 Markdown 原文复制到剪贴板
  （流式进行中取当前已生成部分）
- **思考过程（Ctrl+T）**：推理模型的思考默认折叠为一行 `✻ 思考：摘要 · 用时`；
  `Ctrl+T` 展开——思考进行中在状态栏上方**实时尾随**显示最近若干行，思考定稿后
  内联展示该步思考正文（超 40 行截断标注），再次 `Ctrl+T` 收起；空闲时按
  `Ctrl+T` 把上一步思考**全文**以面板打印。恢复会话时从消息内的 reasoning 块
  还原思考记录（旧会话无此内容则保持原样不显示）
...
- **工具调用卡片**（官方呈现层 `presentCall/presentResult`）：命令卡**紧凑单行**
  （命令截断到一行 + 耗时/退出码 + `输出 N 行`标记，完整命令与全文输出走 `Ctrl+O`
  面板；长命令不再把转写冲出视口）、文件改动卡（红绿 diff 着色）、搜索卡、
  读文件卡、web 卡
- **工具详情切换**：`Ctrl+O` 在摘要/完整输出之间切换（对已定稿的卡，切到完整
  即把最新一张的全文打印成面板；命令卡的面板先印完整命令、再印全文输出）
