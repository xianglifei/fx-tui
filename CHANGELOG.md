...
### 功能：思考过程可折叠展示（Ctrl+T）——推理内容不再只有一行摘要

**动机**：推理模型的思考此前只以一行 `✻ 思考：摘要 · 用时` 呈现（状态栏另有
实时字数），模型正在想什么、方向对不对，长思考期间无从判断；定稿后思考全文
即被丢弃。对照 ZCode TUI 的思考折叠行（`+ Thinking...`，可展开看全文），取其
「默认一行、按需展开」的形态，按 fx-tui 的 Static scrollback 架构另作实现。

**机制**：`Ctrl+T` 三层展开，切换态即所见。

- 实时：思考进行中，状态栏上方出现一行暗色提示 `✻ 思考中…（Ctrl+T 展开思考）`；
  按 `Ctrl+T` 后替换为「头部行（字数/时长）+ 最近 8 行尾随视图」，随思考流
  滚动，再按收起。纯思考阶段此前从不触发渲染提交（flush 只认文本增量），
  状态栏字数也因此不跳动——现按同一 60ms 周期一并驱动，顺带修复。
- 定稿：展开态下每步思考结算为内联暗色块（`✻ 思考 · 用时 · 字数` + 正文，
  超 40 行截断并标注），之后每步同款；收起态保持原一行摘要不变。
- 追看：空闲（或轮次被中断）后按 `Ctrl+T`，把最近一步思考的全文以面板打印
  （视口截断），与 Ctrl+O 工具详情同一惯用法——Static 区已定稿行不可重绘，
  回看只能一次性打印，不能原地展开。
- 存量兼容：恢复会话时从消息内 `reasoning` 块还原思考记录（无实测时长，记
  0ms）；旧会话无此块则原样不显示。思考全文内存封顶 100k 字符（头锚定），
  字数统计不受封顶影响。会话切换重置展开态。

**验证**：单测 359 项全绿（新增 store 思考结算/开关/封顶/回放 10 项、estimate
思考行 4 项）；`pnpm typecheck`、`pnpm lint`、`pnpm build` 通过。

### 功能：bash 命令卡紧凑单行——转写不再被命令原文冲走

**动机**：terminal 呈现层把**完整命令原文**放进卡片标题（`TerminalCallView.title`
即"the command"），fx-tui 侧又不截断、任其 wrap——一条长管道命令占 6 行，再加
5 行输出预览和截断标记，一张 bash 卡 8~12 行；工具密集的轮次里，用户的问题和
模型回复被冲出视口。对照 ZCode（工具卡默认一行短摘要、完整内容收在展开层），
把「一行摘要」换到默认层。

**机制**：摘要模式下 bash 卡收为两行——`✓ <命令截断到终端宽度> · 52ms · exit 0`
+ `输出 N 行（Ctrl+O 查看全文）`（无输出则只剩头行，一张卡最低 1 行）。完整
命令原文与全文输出由 `Ctrl+O` 面板承接：面板正文现在**先印完整命令、再印全文
输出**（此前只印输出，且面板标题不截断）。预先 `Ctrl+O` 切到完整模式的卡片
保留原 400 行 head+tail 输出预览不变。命令截断由视图与高度估算共享的
`terminalHeaderCommand` 完成（CJK 宽度感知，并为尾部 `…` 预留单元格），
Static 区「估算逐行精确」的约束不受影响。diff / 搜索 / 读文件 / web / 通用
卡维持原样。

**验证**：单测 365 项全绿（新增 terminal 紧凑卡高度与截断助手 4 项、Ctrl+O
面板含完整命令 2 项）；`pnpm typecheck`、`pnpm lint`、`pnpm build` 通过。

## [0.34.0] - 2026-09-21

### 功能：流式输出原地生长——动态区重排，消除「从底部跳到顶部」的顿挫

**动机**：对话早期（视口尚有弹性空白时），用户的输入直接出现在横幅下
方，而大模型的流式输出却被弹性 filler 顶在屏幕底缘（状态栏上方）生
成，回复定稿转入 Static 区的瞬间再「跳」到用户消息下方——视觉上像文
字从底部生成后快速上浮，空白越大跳得越显眼；对话超过一屏后 filler 归
零，跳动感才消失。

**机制**：动态区内部重排——流式回复、工具行、待确认卡片、待办面板、
排队托盘等活内容整体移到 filler **之前**，filler 移到活内容与钉底的输
入块（状态栏 + 输入框）之间。高度数学不变（filler 公式与整帧占满视口
的约束原样保留），变的只是绘制顺序：流式输出现在紧贴定稿内容向下生
长，回复定稿时已经在目标位置隔壁，转入 Static 区零位移——与 Claude
Code 的内容流动方式一致（内容自上而下，空白留在内容与输入框之间）。

**验证**：单测 346 项全绿（布局测试只断言高度、不断言顺序，无需修
改）；`tui-stability-matrix.sh` 真机矩阵 54 项断言全部通过（输入框钉
底、横幅顶缘、零 frame-overflow、无振荡四类不变量）；另以 tmux 实发
一条消息抓流式/定稿两帧比对——回复贴着用户消息原地生长，两帧逐字节
一致。

## [0.33.0] - 2026-09-21

### 功能：退出即净屏—— farewell 告别行 + 会话恢复提示

**动机**：fx-tui 全程把对话渲染进主屏 scrollback（换取会话期间的原生滚
轮回看与复制），副作用是退出后 transcript 整屏残留，每次都得手动补一个
`clear`。对照 Claude Code / OpenCode 的体验（退出后终端干净、只留一条
resume 提示），取其「退出净屏」的结果、不动「主屏渲染」的根基——不切
备用屏幕、不自建历史浏览，保持零新增依赖。

**机制**：

- 新增 `farewell.ts`：Ink 卸载、会话 flush 完成后，向终端写入
  `ESC[H ESC[2J ESC[3J`（光标归位 + 清视口 + 清 scrollback，等价于手
  动 `clear`），随后打印两行告别信息：版本与「会话已保存」确认、
  `fx --resume <会话id>` 恢复提示（会话 id 缺失时省略提示行）；
- **只挂在最终退出**：`shutdown()` 新增 `respawn` 标记，`/restart` 的
  重生路径跳过净屏——子进程要无缝接管同一块终端，清屏会把本次对话已
  滚入 scrollback 的部分一并抹掉；启动路径（`wipeViewport`）保持原契
  约不变（只 `ESC[2J`、保留启动前的 shell 历史），两侧语义互不越界；
- 非TTY 场景（管道、重定向）跳过转义序列只写文本，测试以注入 writer
  断言字节级序列。

**影响**：Ctrl+C 双击退出与命令层 `/exit` 类路径退出后，屏幕不再残留
对话，改为「净屏 + 两行告别」；会话本就持久化，恢复方式与此前一致。
放弃的是「退出后回滚屏幕复制历史对话」的能力——与本就手动 `clear` 的
习惯一致，若需留存请退出前复制或直接 `fx --resume` 回看。
...
`@` 补全此前只列文件——「让模型看看这个模块/目录」的场景得自己手敲路径。
现在工作区遍历把目录也收进候选（自动带尾斜杠，如 `src/ui/`），与文件同
过一套 fuzzy 评分器（尾斜杠按展示符号处理，basename 加成照常生效，`@ui`
能命中 `src/ui/`）；菜单中目录标注「目录」，选中后**不补尾空格**，可以
直接继续输入子路径（`@src/ui/` + `Input` → `@src/ui/Input`），文件行为
不变。`node_modules` 等忽略目录照常剪枝。

测试 303 → 305 条（评分器尾斜杠语义、遍历含目录与剪枝、目录在混排候选
中的排名）。

## [0.28.0] - 2026-09-19

### 功能：`!` 输出暂存盘——手动执行的命令结果进入对话上下文

**动机**：v0.27.0 的 `!` 直通只把输出画在转录卡片里，会话日志不留痕迹，
后续轮次模型完全看不到。但真实场景恰恰相反：agent 因权限/沙箱跑不了
（或需要交互、含密钥不便代跑）的命令，用户手动执行后，**期望的正是
模型看着输出给建议**，而不是自己盯着手终端结果——那样手势的价值有限。

**机制（输出暂存盘）**：`!` 命令执行完毕后，除结果卡片外，输出自动
**暂存**进输入框上方的托盘（`🧾 已暂存 N 条命令输出，将随下一条消息
发送` + 每条一行摘要），用户随后输入的消息提交时，暂存块作为独立
text block 拼进同一条用户消息（上下文在前、请求在后）——模型在同一轮
看到结果并给建议，**不触发任何自动回复**，也无需 dsh 提供新注入口；
消息气泡以 `🧾` 行回显暂存摘要，用户能确认模型将收到什么。

- **`!` / `!!` 语义归位**：`!` ＝进上下文（搭下一班车），`!!` ＝仅本地
  查看——0.27.0「两者等义」的临时妥协就此消解，对齐 MiniMax Code 的
  本义；
- **键位与图片附件盘合一**：空输入框 `⌫` 按真实暂存顺序撤销最新的
  一条（图片或输出），`⌥⌫` 清空两盘；提交时随消息一起消费；
- **体量与预算**：暂存块独立于展示卡片做头尾截断（合计 8,000 字符，
  中段以省略行标注），模型读得全而不灌爆上下文；托盘行数计入
  input-height 估算（`outputTrayRows`），用户消息的 `🧾` 行计入
  item 高度估算，filler 预算与渲染继续锁步；
- **不改变会话语义**：输出仍不直接写会话日志——它随用户消息的
  content 进模型、随消息事件落日志，fork / resume / 重放天然正确；
  `!!` 的输出则完全不留痕。

测试 290 → 303 条（store 输出盘增删改与跨盘撤销顺序、echoUser 携带
输出的即时回显与排队晋升、stash 传输块的自描述格式与截断、`!`/`!!`
分流）；tmux 真机验证托盘渲染、`⌫` 撤销提示与再次暂存。

## [0.27.0] - 2026-09-19

### 功能：LLM 重试可视化 · /copy 剪贴板 · !/!! shell 直通（MiniMax Code 启示落地第一批）

**LLM 重试可视化**：接入 dsh-llm-retry 的持久 `llm/retry` /
`llm/retry-started` 会话事件——此前请求失败进入退避重试期间屏幕上只有
一个 spinner，用户分不清「卡死」还是「在重试」。现在状态栏以琥珀色
实时显示 `⟳ 重试 N/M · Xs（原因码）`（无上限策略省略 M），转录同时落
一行警告记录原因码与 provider 错误摘要；重试等待结束（下一次尝试在
途）即清除瞬态状态。回合开始/结束与会话切换兜底清空；历史重试只在
/trace 可查，重放不生成转录行（恢复会话不再被陈旧失败行刷屏）。
devDependencies 新增 `@deepseek-ai/dsh-llm-retry`——纯类型载体（声明
合并注册 `llm/retry` 事件族），版本随 dsh 家族锁定 0.1.5-rc.2，零新增
运行时依赖。

**`/copy` 复制最后回复**：把最后一条助手回复的 Markdown 原文复制到系统
剪贴板（pbcopy）；流式进行中回退为当前已生成的部分；无回复时提示。
终端划选复制会夹带渲染残留与折行，剪贴板往返是单条回复唯一的干净导出
路径。

**`!` / `!!` shell 直通**：输入框以 `!` 开头即在本地 shell
（`$SHELL -c`，工作区目录）直接执行命令，不惊动模型、不写会话日志，
结果以面板卡片呈现（✓/✗ 退出码或信号 · 耗时，输出头尾各半截断——
shell 报错多在末尾，纯头部截断会藏住关键行）；超过 10 分钟的命令杀掉
整棵进程树并标注超时，超过 400ms 才落一行「正在执行」标记（快命令
只出现结果面板）。`!!` 目前与 `!` 执行等义——对齐 MiniMax Code 的
肌肉记忆，但其「输出不进上下文」语义在 dsh 上需等不打断轮次的注入口
（followup 会触发回复、手写会话事件会破坏轮次平衡），暂不承诺。
配套补全：命令位补 PATH 命令名（含工作区 `node_modules/.bin`，30s
缓存），其余词补真实文件系统路径（`~` 展开、目录尾斜杠、含空格自动
单引号包裹；补全标签保留用户输入的 `~/` 原文、不重写为绝对路径）；
菜单内 Enter＝按原文执行、Tab＝补全；命令行进 ↑ 历史（Alt+↑ 取回不受
影响）。mcode 的同名手势即本次三项启示的来源，见 0.26.0 条目的 pi-tui
渊源。

单元测试 250 → 290 条（新增 shell-bang 解析/执行/超时/截断、补全词法
与真实文件系统、/copy 各路径、store 重试事件的实时/重放/复位矩阵）；
tmux 真机冒烟验证直通执行（退出码与输出正确）、补全菜单与 Tab 应用。

## [0.26.0] - 2026-09-19

### 功能：输入框稳定性真机测试矩阵 + 预算感知编辑器封顶

**真机测试矩阵**（`scripts/tui-stability-matrix.sh`）：tmux 固定几何 +
`capture-pane` 抓真实屏幕 + `FX_TUI_DEBUG` 溢出日志，围绕四条不变量
断言——输入框底边框钉在倒数第二行（splash 精确、滚动阶段 ±1 观测）、
banner 钉顶、无帧溢出、双次抓屏逐字节一致（无振荡）。14 个用例覆盖
启动、长行换行、清空、菜单开合、封顶粘贴、菜单叠封顶草稿、菜单快速
循环、快速大块输入、resize 重建、filler 耗尽后的滚动阶段与真实提交；
场景组间重启会话以保证密封（溢出会把 ink 光标模型永久打歪，不能让
后续用例继承坏基线）。需要 tmux 与全局 `fx`，退出码即失败断言数。

矩阵首跑即抓到两个问题并修复：

- **预算感知编辑器封顶**（修 TC6）：v0.25 的编辑器封顶是绝对值，对
  周围空间无感知——封顶草稿（12 行）+ 斜杠菜单（11 行）+ banner
  （8 行）+ 固定区（4 行）在 30 行视口下照样溢出，且闪烁检测器只测
  动态区高度、对此**失明**。现在 App 在同一 commit 内先算 settled
  高度与其余动态区行数（`computeSettledHeight` /
  `computeFixedLive`，原 `computeFiller` 拆分），新增
  `editorRowsForSpace` 从剩余空间刨出编辑器可见行（预留溢出指示行 +
  一行安全余量），菜单/附件盘/流式回复长一寸、编辑器缩一寸，输入框
  底边框始终钉底；`InputBox` 改收 `editorVisibleRows`（预算结果），
  估算与渲染消费同一数字，锁步不变。
- **重建后 filler slack 残留**（修 TC8）：resize 重建首帧的 2 行安全
  slack 此前会一直烙在已绘制的帧上（输入框悬在底行上方 2 行，直到
  下一次无关渲染才纠正）——现在 `firstFrameRef` 翻转后在同一布局
  阶段同步调度一次无 slack 的纠正 commit。

**结构性发现（如实记录，根治属渲染器级工程）**：滚动阶段（filler 耗
尽、banner 滚出视口）下，stock ink 的帧高计数与终端物理光标永久分叉，
后续 Static 追加会令整帧上跳（跳变量 = 此前滚动量），输入框脱离底部
7~10 行；80×30 与 100×40 下同序列表现不同纯属重写路径运气。闪烁检
测器对此失明（动态区高度恒 ≪ 视口，日志 0 条与肉眼跳变并存）。矩阵
将滚动阶段底边框断言降级为 INFO 观测值，完整机理与 100×40 复现序列
见 `docs/ui-research-2026-08.md` 第五节；根治需自研 diff 渲染器 /
fork ink / alternate screen（同文档第一节），与 MiniMax Code 实证的
pi-tui 路线同归。
...
- `trace.test.ts` 对 `/tree` 面板首行的断言里写死了字面时间
  `1-2 11:04`——该值来自 `Date.UTC(...)` 在**运行机器时区**下的渲染，
  UTC+8 的开发机上是 11:04，UTC 的 CI runner 上是 03:04，于是
  `pnpm test` 在本地通过、在云端 `verify` 作业上必失败
- 改为只断言时间戳的**形状**（`\d+-\d+ \d{2}:\d{2}`），具体数值交给
  真实时钟；这正是全仓库最后一处此类断言，其余测试均只断言不含时区
  的字段
- 顺带核对了 CI 本身：check-run 名字 `verify` 与分支保护要求的
  `contexts: ["verify"]` 一致，配置无误——此前推送时的
  "Bypassed rule violations" 只是检查尚未跑完就被管理员权限放行
...
- **/btw 运行中守卫**：此前不检查 agent 是否空闲，运行中调用
  `deriveMessages()` 拿到的是半截消息面；现在 busy 时直接提示拒绝
- **悬空瀑布 fail-closed**：两桥新增 `cancelQuiet()`（免通知、免提交的
  清理路径），`TuiStore.reset()` 与 `dispose()` 开头调用——会话切换或
  退出时仍挂起的审批/问答 promise 不再永久悬空（上游 abort 路径正常
  情况下已先行清理，此处为防御式兜底，新增两个回归测试锁死）
- **AppleScript 转义**：完成通知的文本在拼入 `display notification` 字符串
  字面量前转义反斜杠与双引号——当前只含固定文案和数字，但一旦掺入模型
  片段不会再被引号截断
- **更新锁释放校验**：`releaseLock` 先比对锁文件里的 pid 是否为本进程，
  只释放自己持有的锁——此前若偷到「陈旧」锁的持有者只是 hang 而非死亡，
  慢持有者事后释放会误删新持有者的锁，放行第三个实例并发更新
...
- **动机**：`walk()` 用 `readdir(root, { recursive: true })` 一次性递归读完整棵
  树，然后才逐条过滤——`node_modules/`、`.git/` 等在上万条 Dirent 已经物化进
  内存之后才被跳过，大仓首次 `@` 补全会明显停顿；`MAX_FILES` 截断发生在遍历
  循环里，为时已晚
- **修复**：改为手写 DFS（`withFileTypes` 逐目录读）：读到一个条目就判断，
  忽略名单命中的**目录根本不入队**——它们的内容从头到尾不存在于内存；目录
  符号链接不入队（防循环）。相对路径由 `path.relative` 直接得出，替换原先的
  `parentPath` 前缀剥离
- **行为兼容**：排序（长度优先 + locale）、`MAX_FILES` 上限、TTL 缓存与
  `inflight` 去重全部保留；文件名恰为忽略段（如名为 `build` 的文件）仍被跳过，
  与原先按路径段过滤的语义一致
- **影响面**：大仓 `@` 补全首次响应显著加快；新增剪枝测试（临时目录构造
  node_modules/.git/dist/src，断言只收 src 与根文件）
