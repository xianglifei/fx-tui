...
export const FX_TUI_VERSION = '0.35.0'
...
! <命令> 终端直通执行：本地运行并显示结果，输出自动暂存、随下一条消息进入对话上下文
  （!! 则仅本地查看不进上下文；空输入框 ⌫ 撤销最后一条暂存，⌥⌫ 清空）
...
function bottomFlush(): void {
  ...
  const actions = {
     ...
     * (Tab) always books its own next turn. The ride-along trays — attached
     * images and stashed `!` outputs — are consumed here: output blocks go
     * BEFORE the typed text so the model reads context first, request second. */
    onSubmit(text: string, opts?: { queue?: boolean }): void {
      debugLog('submit', text)
      const images = store.consumePendingImages()
      const outputs = store.consumePendingOutputs()
      const content: ContentBlock[] = []
      for (const output of outputs) content.push({ type: 'text', text: output.text })
      ...
      store.echoUser(
        ...
        text,
        {
          ...
          outputs: outputs.map(output => output.summary),
          ...(steer ? { mode: 'steer' as const } : { mode: 'queue' as const }),
        },
      )
    ...
    /** `!` shell passthrough: run locally, show the outcome as a panel, and
     * (unless `!!`) stash the output for the next submitted message so the
     * model sees it without firing a turn of its own. The raw line (bang
     * included) rides the input history so ↑ recalls it. */
    onShell(line: string): void {
      debugLog('shell', line)
      const bang = parseShellBang(line)
      const command = bang?.command ?? ''
      if (command === '') return
      inputHistory.push(line)
      void launchShellPassthrough(command, store, { stash: bang?.quiet !== true })
    },
