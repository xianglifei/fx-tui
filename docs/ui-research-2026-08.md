...
## 五、2026-09 补充：MiniMax Code 实证与滚动阶段结构性限制

### MiniMax Code（开源）验证了 Pi 渲染器路线

`MiniMax-AI/minimax-code` 的 TUI 整体构建在 vendored 的 pi-mono
（earendil-works Pi，MIT）之上——即本文描述过的「CSI 2026 + 行级 diff +
显式光标追踪」一派。生产级实证要点：每帧全量逻辑渲染成行数组（组件按
text+width 缓存）→ 渲染器行级 diff → 同步输出原子上屏；帧超视口、diff
落在视口上方等一切兜不住的情形直接 fullRender + 清 scrollback 自愈。
**稳定性是渲染器的职责而非应用的职责**——本文第三节记录的整个「估算器 +
filler 预算」体系本质上是在替 stock ink 兜底，在 pi-tui 架构下结构性不
存在。其编辑器同样封顶（视口 30%、下限 5 行）+ 内部滚动 + `↑ N more`
边框指示，与本仓库 v0.25 的编辑器封顶同构。

### 滚动阶段的结构性失步（stock ink 无法根治，已建复现）

真机矩阵（`scripts/tui-stability-matrix.sh`）在 100×40 下发现：banner
被大面板挤出视口（filler 归零、进入滚动阶段）后，再追加 Static 条目会
令整帧上跳（跳变行数恰等于此前的滚动量，如 /help 后 9 行），此后每次
追加继续漂移——输入框底边框脱离视口底部 7~10 行。机理：ink 的帧高
计数（frame rows）与终端物理光标（viewport rows）在帧超出视口后永久
分叉，ink 以帧坐标做相对移动，终端按屏幕坐标执行。这正是第一节所述
「帧溢出 → ink 失步 → 永不自愈」家族的滚动阶段形态：

- splash 阶段（filler > 0）帧恒 ≤ 视口，本仓库的全部纪律都在阻止溢出；
- 滚动阶段帧**必然** > 视口，stock ink 无从修复；
- 80×30 与 100×40 下同序列表现不同（前者稳定）纯属 ink 重写路径差异。

闪烁检测器对此**失明**（它测的是动态区 yoga 高度，恒 ≪ 视口）——
`frame-overflow` 日志 0 条与肉眼可见的整帧跳变并存。根治只有渲染器级
方案（自研 diff 渲染器 / fork ink / 进 alternate screen，见第一节），
短期以测试矩阵将该行为固化为可观测基线。

### 编辑器预算感知封顶（v0.26）

v0.25 的编辑器封顶是绝对值（视口 30%），对周围空间无感知：封顶草稿
（12 行）+ 斜杠菜单（11 行）+ banner（8 行）+ 固定区（4 行）= 35 行 >
30 行视口，菜单一开照样溢出（矩阵 TC6，且检测器失明）。v0.26 把封顶
改为**预算感知**：App 在同一 commit 内先算 settled 高度与其余动态区
行数，`editorRowsForSpace` 从剩余空间里刨出编辑器可见行（预留指示行 +
安全行），再定输入框总高——菜单/附件盘/流式回复长一寸，编辑器缩一寸，
输入框底边框始终钉底。
